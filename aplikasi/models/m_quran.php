...
class M_quran extends CI_Model {
	...
	function m_twitterShare($ayatId = 1) {
        ...
        foreach ($query->result() as $key) {
			...
			$urlEncode = urlencode($urlAyat);
			$datatext = '['.$key->SuraID.':'.$key->VerseID.'] '.$key->nama.' ('.$key->arti.'):Ayat '.$key->VerseID.' - '.$key->arab;
			return '<a href="https://twitter.com/share" class="twitter-share-button" data-url="'.$urlAyat.'" data-text="'.$datatext.'" data-via="novaherdi" data-hashtags="indoquran.web.id" data-size="large">Tweet</a>
