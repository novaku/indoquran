...
class M_quran extends CI_Model
{
	...
	function m_insertLogPengunjung()
	{
		...
		if ((empty($visId)) && ($this->agent->is_robot() == false)) {
			...
			for ($i = 0; $i < count($filters); $i++) {
				...
				switch ($filterType) {
					...
					case 'date' :
						switch ($compare) {
							case 'eq' :
								$this->db->like($field, date('Y-m-d', strtotime($value)), 'after');
								Break;
							case 'lt' :
								$this->db->where($field . ' < ', date('Y-m-d', strtotime($value)));
								Break;
							case 'gt' :
								$this->db->where($field . ' > ', date('Y-m-d', strtotime($value)));
								Break;
	...
	function m_getListSurahId()
	{
		...
		if ($usememcached) {
			...
			if ($memGetListSurahId) {
				...
				goto save_m_getListSurahId;
			}
		}

		save_m_getListSurahId :
	...
	function m_getListAyatId()
	{
		...
		if ($usememcached) {
			...
			if ($memGetListAyatId) {
				...
				goto save_m_getListAyatId;
			}
		}

		save_m_getListAyatId :
	...
	function m_autoCompleteSearch()
	{
		...
		if ($usememcached) {
			...
			if ($memGetAutoComplete) {
				...
				goto save_m_autoCompleteSearch;
			}
		}

		save_m_autoCompleteSearch :
		$this->db->start_cache();
	...
	function m_data_site_map()
	{
		$url = array ();
		$query = $this->db->select('ID')->from('quran_indo')->order_by('ID')->get();
		foreach ($query->result() as $key) {
			$url[] = "quran/viewAyat/" . $key->ID;
		}
		return $url;
	}
}