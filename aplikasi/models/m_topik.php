			...
			foreach ($arrIsi as $val) {
				...
				foreach ($q->result() as $r) {
					// $hasil .= '
						...
						// </table>
					// ';
					$penjelasan = $r->AyahPenjelasan == '' ? '' : '</tr><tr><td><br/>'.$r->AyahPenjelasan.'</td></tr>';
					$hasil .= '
						<tr>
							<td valign="top">('.$i.') QS. ['.$r->SuraID.':'.$r->VerseID.'] => <a style="cursor:pointer" name="btn'.$r->ID.'" onClick="javascript:viewAyat('.$r->ID.')"><img src="'.base_url().'assets/images/ico/book_open.png"/> Buka Ayat ['.$r->SuraID.':'.$r->VerseID.']</a><br/><b>"'.$r->AyahTextNew.'</b>"</td>
							'.$penjelasan.'
						</tr>
						<tr><td colspan="2"><hr noshade size="1"></td></tr>
					';
